...
df_new = df_new.drop(columns=['animal', 't', 'ch', 'isi_cv',  'br', 'bdur_max', 'bdur',
                              ...
                              'ibi_cv', 'd',  'd_max', 'r' ])  # dropped  # dropped

#  'bf','sync_n', 'sfr', 'r_max', 'bf_deviation' not dropped
...
df1_new = df1_new.drop(columns=['animal', 'loc', 'ch', 'isi_cv', 'br', 'bdur_max', 'bdur',
                              'nspikes_burst_max', 'nspikes_burst', 'p_bursting_time', 'p_bursting_spike',
                              'ibi_cv', 'd_max', 'd', 'r'])  # dropped

#  'bf', 'sync_n', 'r_max','sfr', 'bf_deviation' group  not dropped
...
self_training_model = SelfTrainingClassifier(model, criterion='k_best', max_iter=20)
self_training_model.fit(X, y)
print(self_training_model.score(X_test, y_test))

pickle.dump(self_training_model, open('models/self_training', 'wb'))
